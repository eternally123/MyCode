...
private:
    void do_read()
    {
        socket_.async_read_some(boost::asio::buffer(data_, max_length),
                                std::bind(&session::readHandle, this, std::placeholders::_1, std::placeholders::_2));
    }

    void do_write(std::size_t length)
    {
        boost::asio::async_write(socket_, boost::asio::buffer(data_, length),
                                 std::bind(&session::writeHandle, this, std::placeholders::_1, std::placeholders::_2));
    }
...
private:
    ...
    std::vector<std::shared_ptr<session>> session_lists_;
};
