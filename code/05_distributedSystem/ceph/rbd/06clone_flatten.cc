...
int main(int argc, char *argv[])
{
    ...
    ret = rbd.open(io_ctx, child_image, "image01_child");
    if (ret < 0)
    {
        err_exit(ret, "open clone image failed");
    }
    ret = child_image.flatten();
    if (ret < 0)
    {
        err_exit(ret, "flatten failed");
    }
    else
    {
        std::cout << "flatten clone image success\n";
    }
