...
int main(int argc, char *argv[])
{
    ...
    librados::bufferlist write_bl;
    librados::bufferlist read_bl;
    std::string msg = "hello world";
    write_bl.append(msg);

    image.write(0, msg.size(), write_bl);
    ret = image.read(0, info.size, read_bl);
    std::cout << "read " << ret << " bytes from rbd,actually get " << strlen(read_bl.c_str()) << " bytes:" << read_bl.c_str() << "\n";
